...
def login():
    ...
    if admin != 1:
        ...
        session['admin_expire_date'] = int(time.time()) - randint(10, 266) * 24 * 60 * 60 #current minus between 10 and 265 days
    expire_date = int(time.time()) + 30 * 24 * 60 * 60 #current time + 30 days
...
def get_flag():
    ...
    try:
        ...
        if 290 * 24 * 60 * 60 < abs(int(token["expires"]) - session['admin_expire_date']) < 300 * 24 * 60 * 60: #se |expire date cookie - exp date sessione| è compresa fra 290 giorni e 300 giorni  (la distanza può assumere i valori da 40 a 295)
